...
from ..core import response_policy
from ..frontend import STATIC_DIR, app_html
from ..kb import knowledge_graph as kg
from ..observability import llm_trace
from ..observability import trace_viewer
...
async def ask(req: AskRequest) -> AskResponse:
    ...
    answer_kwargs = {"mode": mode, "repo": repo}
    if question_type:
        answer_kwargs["question_type"] = question_type
    try:
        result = await _run_governed(lambda: agent.answer(question, **answer_kwargs))
...
async def diagnose_endpoint(req: DiagnoseRequest) -> DiagnoseResponse:
    ...
    from ..diagnostics import diagnose as diag
    repo = _resolve_request_repo(req.repo)
...
async def knowledge_qa_ask(req: KnowledgeCurateAskRequest) -> dict:
    ...
    if question_type:
        answer_kwargs["question_type"] = question_type
    try:
        answer = await _run_governed(lambda: agent.answer(question, **answer_kwargs))
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"上游模型调用失败: {exc}")
