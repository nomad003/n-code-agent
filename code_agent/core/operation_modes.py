...
7. 如果是配置类问题，首答必须给一个短表格，列出配置面、对应配置表/配置项、核心字段和字段用途；表名和字段名属于业务配置线索，可以保留。
8. 如果是功能类问题，首答给 Mermaid 流程图 + 结构化步骤；只说明配置作为输入从哪里影响流程，不展开配置字段明细，除非用户明确追问字段。
...
3. 如果是配置类问题，先给配置面、对应配置表/配置项、核心字段和字段用途；再补加载链路、使用位置、默认/非法值和验证方式。
4. 如果是功能类问题，先给 Mermaid 流程图 + 结构化步骤；再补关键类、函数、调用链、数据结构、边界条件和风险点。
