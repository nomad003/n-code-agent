...
4. 用户只问“怎么配置/是什么/怎么排查/整体流程”时，默认输出 4 到 6 条纲要；不要列出知识卡、文件路径、类名、函数名、日志短语、断言名等内部线索。
5. 只有用户继续追问具体配置字段、具体日志、具体错误或明确要求代码位置时，才逐步展开对应细节；仍然避免一次性倾倒所有匹配信息。
6. 如果信息不足，先说明缺口和需要的输入，不要用长篇解释填充。
"""
    if mode == "technical":
        return """\
