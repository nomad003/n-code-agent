...
7. Mermaid 图里的条件分支必须保留代码真实判断方向，例如 `!= 0`、`== 0`、空/非空不能反写；不确定时标注“待核实”而不是猜。
8. 如果问题里出现类似 XConfig/XTable/XLimit/XFollow/Statistics/Presentation 的配置表或字段名，要同时说明配置加载文件和运行时使用文件。""",
    "config_impl": """\
最佳实践：
...
6. Mermaid 图里的条件分支必须保留代码真实判断方向，例如 `!= 0`、`== 0`、空/非空不能反写；不确定时标注“待核实”而不是猜。
7. 如果还没确认到具体表名或字段名，必须明确标注“待核实”，并继续用工具查证；不要只输出抽象模块说明。
8. 若字段只在数据表中出现但代码未读取，要明确说明“未找到代码使用点”。""",
    "general": """\
最佳实践：
