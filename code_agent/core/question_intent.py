...
6. 输出结构上，重点给 Mermaid 流程图、结构化步骤、参与模块、状态变化和关键分支；配置只作为输入点说明，不展开字段明细，除非用户追问配置。
...
5. 输出结构上，首答要给配置明细短表格，建议列为：配置面、对应表/配置项、核心字段、字段用途；可配一张 Mermaid 关系图说明表之间如何协同。technical 模式再补加载文件、函数名和代码位置。
6. 若字段只在数据表中出现但代码未读取，要明确说明“未找到代码使用点”。""",
    "general": """\
最佳实践：
