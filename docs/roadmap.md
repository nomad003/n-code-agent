...
**渐进路径：**
- **MVP**：问答后存 `{问题, 答案, 引用文件+哈希}` 到 SQLite；下次新问题先关键词/向量召回，命中则把旧答案作为线索注入。先测**命中率与正反馈是否成立**。
- **V2**：加失效（源文件变 → 标 stale）、可信度、`recall_knowledge` 工具。
- **V3**：与方案 2 的 AST 符号表合并成统一检索；知识去重/合并。
...
- 价值：高（飞轮成立时延迟/成本/质量持续改善）；成本：高（依赖方案 2 检索层 + 失效机制 + 需先验证飞轮有效）。
- 依赖：建议在方案 2（A）落地后做，复用其检索层。
...
### F. 运行时诊断：日志 / coredump backtrace 分析（新能力方向）
...
- 价值：高（gameserver 运维刚需，超出纯代码问答）；成本：中-高。
- 依赖：符号映射强依赖方案 2 符号索引；版本绑定与方案 3 共用机制。建议方案 2 之后启动。
