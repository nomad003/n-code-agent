...
| mode | 作用 |
|------|------|
| `plain` | 面向非程序员/策划/QA，采用渐进披露；优先用表格、短清单和 Mermaid 图表达。用户问题涉及配置时必须给“配置面/表/字段/用途”明细表；涉及功能时必须给 Mermaid 图解和结构化步骤；不输出代码、命令、JSON、配置样例，也不默认暴露知识卡/文件/符号等内部线索 |
| `technical` | 面向程序员，同样先用表格、短清单和 Mermaid 图表达；用户问题涉及配置时必须先给表/字段/用途，再补加载链路和使用位置；涉及功能时必须先给 Mermaid 图解和结构化步骤，再补类、函数、调用链、文件路径和行号 |
...
| 需要澄清 | “看一下/怎么弄/这个问题/怎么配置”等缺少对象或目标的输入 | 不调用工具 | 追问用户要查 crash、日志、配置、功能还是通用解释，并说明需要补充的输入 |

如果没有命中这四类且问题仍有明确对象，会注入通用策略：先归类、低成本缩小范围、区分事实和推断。
如果自动识别判断意图不明确，`agent.answer()` 会在进入 LLM/tool loop 前直接返回澄清问题。
