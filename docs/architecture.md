...
| `shortcut.py` | 入口短路：精确"X 定义在哪"直接查索引返回、跳过 LLM（方案 2） |
...
2. **方案 2（符号索引已落地）**：`indexer.py` 用 tree-sitter 解析 C++ → SQLite 符号表 + FTS5 全文索引；`tools.find_symbol`/`grep_code` 优先走索引（快、精确），无索引则回退 live scan。支持**增量更新**（`indexer.update()` / `scripts/index.sh --update`，按文件哈希只重建变更项）和**入口短路**（`shortcut.py`：精确"X 定义在哪"直接查索引、跳过 LLM）。`tools.py` 作为唯一接触文件的层，索引顶替其实现而 `agent.py` 不动。向量库/语义检索推迟到方案 3。
